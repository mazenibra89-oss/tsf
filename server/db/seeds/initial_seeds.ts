...
export async function seed(knex: Knex): Promise<void> {
  // Check if data already exists in any key table (e.g. event_phases)
  const phasesCount = await knex('event_phases').count('id as count').first();
  ...
  await knex('admin_accounts').insert([
    { username: 'admin', password_hash: defaultAdminPasswordHash }
  ]);
  ...
  // Seed Thrift Vendors
  await knex('thrift_vendors').insert([
    {
      id: 'v-1',
  ...
  // Seed Thrift Products
  await knex('thrift_products').insert([
    {
      id: 'p-101',
  ...
  // Form Questions Config (Pre-populate with initial structure)
  const SEED_FORM_QUESTIONS = {
    dataDiri: [
